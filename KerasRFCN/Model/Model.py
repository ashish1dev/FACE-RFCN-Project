...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, config):
    ...
    # negative_indices = tf.where(tf.logical_and(roi_iou_max < 0.5))[:, 0]

    # 1. Negative ROIs are those with >= 0.5 IoU with a GT box
...
class VotePooling(KE.Layer):
    ...
    def call(self, inputs):
        ...
        sorting_tensor = box_to_level[:, 0] * 12876 + box_to_level[:, 1]  #  100000
