...
def build_rpn_targets(image, anchors, gt_class_ids, gt_boxes, config):
    ...
    ids = np.where(rpn_match == 1)[0] # It returns indices of +ve anchor
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    while True:
        try:
            ...
            image_index = (image_index + 1) % len(df)
